...
JxlToJpegAsyncWorker::JxlToJpegAsyncWorker(Napi::Buffer<uint8_t> &input, size_t input_size, Napi::Function &callback) 
...
void JxlToJpegAsyncWorker::Execute()
{
  ...
  uint8_t* output = jxlDecompress(this->inputPtr, this->inputSize, this->outputSize);
  if (!output) {
...
Napi::Value jxlToJpeg(const Napi::CallbackInfo& info) {
  ...
  JxlToJpegAsyncWorker *worker = new JxlToJpegAsyncWorker(buffer, buffer.ByteLength(), cb);
  worker->Queue();
