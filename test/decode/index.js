...
const func = async () => {
  const files = fs.readdirSync('./test/decode/input');

  const result = [];
  for (const fileName of files) {
    const fileUrl = `./test/decode/input/${fileName}`;
    const time = [];
    for (let i = 1; i <= 30; i++) {
      ...
      try {
        ...
        if (i == 1) await writeFile(`./test/decode/output/${fileName.replace(".jxl", "")}.jpg`, image);
